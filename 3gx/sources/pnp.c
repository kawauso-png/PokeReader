...
bool is_memory_mapped(u32 addr) {
  ...
  // Query success also describes unmapped pages. Require read permission
  // before any caller treats this address as readable memory.
  return result == 0 && (info.perm & 1U) != 0;
}
