...
t=rep(t,'    pub fn draw_rng_status(&self) {','''    pub fn draw_rng_status(&self) {
        ...
        if self.practical_miss==16 {
            pnp::println!("S7102 CAPTURE FAILED");
            pnp::println!("NO UP - RESET VC");
            pnp::println!("CSV {}",pnp::trace_written_slot());
            return;
        }''')
t=rep(t,'"S786 NEUTRAL SCAN A/r10 B76"','"S7102 OBSERVE SCAN"',2)
t=rep(t,'"S786 NEUTRAL ROOT READY"','"S7102 ROOT READY"')
...
t=t[:a]+'''        } else if self.probe_session && self.probe_active && !self.practical_active {
            pnp::println!("S7102 {} RECORD",super::research::mode_name());
            pnp::println!("WAIT FOR FINAL DV");
            pnp::println!("RELEASE UP AT PAUSE");
'''+t[b:]
t=rep(t,'STALLPHASE,V7100,','STALLPHASE,V7102,')
...
static void v7102_ready_panel(void) {
    char title[32];snprintf(title,sizeof(title),"S7102 %s READY",v7102_name());
...
c=rep(c,'''                arm_suicune_probe();
                ...
                if (!suicune_research_arm_ok()) {
                    ...
                    v7102_panel("S7102 CAPTURE FAILED","NO UP - RESET VC","ERROR SNAPSHOT SAVED IF SD OK");
                    continue;
                }
...
c=rep(c,'''                suicune_start_phase_lock_active = false;
            ...
            // Neutral delay''','''                suicune_start_phase_lock_active = false;
                ...
                if(suicune_live_pass_ready) v7102_panel("S7102 RECORD ARMED","HOLD UP UNTIL PAUSED","THEN RELEASE UP");
                else v7102_panel("S7102 INPUT ARM FAILED","DO NOT PRESS UP","RESET VC MANUALLY");
                continue;
            }
