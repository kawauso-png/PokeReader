...
# v737 rejects every nearest-neighbour extrapolation farther than 16 buckets.
# Keep the reported radius honest: the inherited v736 R128 label otherwise
# implied that far buckets were being accepted when they were not.
oldrad='''    if steps < 2048 { 4 }\n    else if steps < 6144 { 8 }\n    else if steps < 12288 { 16 }\n    else { 128 }'''
newrad='''    if steps < 2048 { 4 }\n    else if steps < 6144 { 8 }\n    else { 16 }'''
assert oldrad in ps, 'adaptive radius anchor missing'
ps=ps.replace(oldrad,newrad,1)
...
print('applied v7.3.8 deduplicated confidence support, radius capped at 16')
