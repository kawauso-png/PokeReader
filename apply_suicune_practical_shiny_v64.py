...
# Generator bug found by CI: lane prototypes are bytes (b'A'/b'B'), so
# chr(proto) raises TypeError on Python 3.13. Patch exactly the known generator
...
# Only validate transport-level markers here. The workflow performs the
# authoritative post-generation marker checks against the actual Rust/C files.
required = ["Practical Shiny", "S64"]
missing = [marker for marker in required if marker not in decoded]
if missing:
    raise SystemExit(f"v6.4 decoded payload missing transport markers: {missing}")
