...
# final Random PCs. We only copy a few ARM registers already supplied to
# gb_read_mem by the hook trampoline.
for marker, label in [
...
# Persist the compact signature only after result lock. Later historical patches
# changed the comments immediately before trace_file_close(), so anchor on the
# final close call inside Trace::save rather than on those comments.
csv_block = '''        // v7.1.5: compact PURETAIL signature captured without Random-time\n        // emulator-memory reads. Persist only after DV/result lock.\n        line.clear();\n        let fast_calls = super::hook::endpoint_fast_tail_calls();\n        let fast_sig_count = super::hook::endpoint_fast_tail_sig_count();\n        let _ = write!(line, "\\nfasttail715,version,calls,samples\\nFASTTAIL715,V715,{},{}\\n", fast_calls, fast_sig_count);\n        pnp::trace_file_write(line.as_bytes());\n        line.clear();\n        let _ = write!(line, "fasttail715_call,index,r1,r2,r3,r4,r5,r12,lr,host_pc\\n");\n        pnp::trace_file_write(line.as_bytes());\n        let fast_shown = (fast_sig_count as usize).min(super::hook::ENDPOINT_FAST_SIG_MAX);\n        for i in 0..fast_shown {\n            let s = super::hook::endpoint_fast_tail_sig(i);\n            line.clear();\n            let _ = write!(line, "FASTCALL,{},{:08X},{:08X},{:08X},{:08X},{:08X},{:08X},{:08X},{:08X}\\n",\n                i, s.r1, s.r2, s.r3, s.r4, s.r5, s.r12, s.lr, s.host_pc);\n            pnp::trace_file_write(line.as_bytes());\n        }\n\n'''
close_marker = '        pnp::trace_file_close();'
pos = t.rfind(close_marker)
if pos < 0:
    raise SystemExit('v715 Trace::save final close marker not found')
t = t[:pos] + csv_block + t[pos:]

# UI epoch only; scan criteria and Stage3 CSV compatibility epoch stay put.
t = t.replace('"S714 ', '"S715 ')

# Static safety assertion on the final Random fast path. regs[] loads and static
# assignments are allowed; timing/emulator reads are not.
