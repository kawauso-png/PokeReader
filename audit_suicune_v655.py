...
# Search-start state must be reusable after ERR2/ERR4/etc.
for marker in [
...
# Late path checks must still be hard guards (do not silently learn past a bad stop2 path).
...
# UI must distinguish adaptive learn and search diagnostics.
for marker in ["S65 LEARN 1", "S65 MISS {}", "S64 ERR {} K{}", "S64 WAIT", "S64 READY"]:
...
print("AUDIT PASS: v6.5.5 runtime invariants verified")
