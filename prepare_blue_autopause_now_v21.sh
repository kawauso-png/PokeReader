...
if ! grep -q 'v7.7.1 fresh NOW' "$FCMOD"; then
    sed -i '/LIVE.scan_age = LIVE.scan_age.saturating_add(1);/i\            // v7.7.1 fresh NOW: AutoPause needs this exact sampled frame.\
            ...
            LIVE.valid = now_n != 0 && next_frame(frame) != 0;' "$FCMOD"
fi
