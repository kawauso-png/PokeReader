...
fn next_sample_sub(sample_sub: u8) -> u8 { sample_sub.wrapping_add(FRAME_SUB_STEP) & 0x3F }
...
fn predict_one(rng: u32, div: u8, candidate: Candidate) -> (u32, u8, Candidate) {
    ...
    (((add1 as u32) << 16) | ((sub1 as u32) << 8), next_div,
...
impl Tracker {
    ...
    fn sub_count(&self) -> u8 {
        let mut mask = 0u64;
        for i in 0..self.count { mask |= 1u64 << self.candidates[i].sample_sub; }
    ...
    fn seed_from_transition(&mut self, seq: u32, div: u8, div_step: u8, first: u8, gap: u8, reason: u8) -> bool {
        ...
        self.last_gap = gap;
        let center = (k as u16) * SUB_MOD;
        let lo = center.saturating_sub(63);
    ...
    fn rebuild_forecast(&mut self, rng: u32, div: u8) {
        ...
        for k in 0..FORECAST_HORIZON {
            ...
            ref_rng[k] = nr; ref_div[k] = nd; r = nr; d = nd; c = nc;
        }
        ...
        let mut prefix = 0u8;
        for v in same { if !v { break; } prefix += 1; }
    ...
    fn observe(&mut self, prev_seq: u32, prev_rng: u32, prev_div: u8,
        ...
        let consecutive = prev_seq != 0 && seq == prev_seq.wrapping_add(1)
            && self.last_seq == prev_seq && self.last_usable;
        if !consecutive {
...
pub fn stats() -> TrackerStats { unsafe { TRACKER.stats() } }
pub fn mark_arm() { unsafe { TRACKER.arm_stats = TRACKER.stats(); } }
pub fn arm_stats() -> TrackerStats { unsafe { TRACKER.arm_stats } }
