...
int main(int argc,char**argv){if(argc!=5)return 2;Shadow7113Input in={0};in.static_data=calloc(0x14f000,1);in.heap_data=calloc(0x100000,1);in.code=calloc(0xb1000,1);in.rom=calloc(0x200000,1);in.rom_size=0x200000;in.rtc=rtc;in.event=event;
 ...
 printf("{\"error\":%u,\"arm_pc\":\"%08X\",\"arm_op\":\"%08X\",\"guest_pc\":\"%04X\",\"frame\":%u,\"normal\":%u,\"final\":%u,\"reads\":%u,\"dv\":\"%04X\",\"shiny\":%u,\"instructions\":%llu,\"cpu_seconds\":%.6f}\n",r.error,r.arm_pc,r.arm_op,r.guest_pc,r.frame,r.normal,r.final,r.reads,r.dv,r.shiny,(unsigned long long)r.instructions,(double)(clock()-start)/CLOCKS_PER_SEC);return r.error?1:0;}
