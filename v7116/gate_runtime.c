...
extern uint32_t suicune_rank_pre_state(uint32_t*);
...
static uint32_t snapshot_saved,snapshot_hashes[4];
#ifdef GATE7113_TEST
...
int gate7113_evaluate(void){
 checks++;error=selected=models=committed=0;snapshot_saved=0;snapshot_ticks=0;memset(snapshot_hashes,0,sizeof(snapshot_hashes));memset(results,0,sizeof(results));
 uint32_t st=suicune_rank_pre_state(&advance);seed=st&65535;if(!(st&0x80000000U)){error=0x711303;return -1;}
 /* Broad historical shortlist only. It may omit true candidates; a rank is
  * NEVER sufficient to stop. Every selected candidate passes native replay. */
 int old=rank7108_evaluate();if(old<0){error=rank7108_error();return -1;}
 if(rank7108_cycles()<8569||rank7108_cycles()>11350||!rank7108_best_rank()||rank7108_best_rank()>2048||!rank7108_support())return 0;
 ...
 /* Save the actual pre-replay buffer before either model mutates it. SD IO
  * failure is diagnostic only; it never manufactures or accepts a candidate. */
 ...
 uint64_t saved_at=svcGetSystemTick();
 snapshot_saved=snapshot7116_save(&meta,(const uint8_t*)0x100000,data,(const uint8_t*)meta.rom_base,snapshot_hashes);
 snapshot_ticks=svcGetSystemTick()-saved_at;
 ...
 uint32_t now=0,current=suicune_rank_pre_state(&now);if(now!=advance||current!=st){error=0x711309;selected=models=0;return -1;}
 return selected?1:0;
}
