...
typedef struct {uint32_t error,arm_pc,arm_op,guest_pc,frame,normal,final,reads,lcd,lcdlong,timer;uint64_t instructions;uint16_t dv;uint8_t shiny;uint32_t dv_write_mask,dv_write_frame;uint32_t fast_enabled,fast_blocks,fast_mismatch;uint64_t fast_steps;} Shadow7113Result;
