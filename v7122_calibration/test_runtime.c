...
#include "runtime.inc"
uint32_t gate7118_export(uint32_t*p){export_calls++;tick+=export_delay;if(hash_change)mac7119_hashes[hash_change-1]++;if(export_fail)return 8;Snapshot7116Meta m=mac7119_meta;m.search_id=tick+export_calls;m.advance=advance;m.seed=state&65535;m.clock_tick=tick;mac7119_exported(&m,mac7119_hashes);*p=38;return 0;}
...
int main(int argc,char**argv){
 ...
 reset();make_request(2);request[18]=3;checksum();assert(!poll()&&mac7119_error==22);
 ...
 for(unsigned dv=0;dv<=65535;dv+=65535){reset();arm_request();request[18]=2;request[22]=dv;checksum();assert(!poll()&&!gate7119_enabled()&&!release_count&&committed&&selected==dv&&arm_calls==1);}
 reset();arm_request();request[18]=2;request[22]=65536;checksum();assert(!poll()&&mac7119_error==22&&!committed);
 for(unsigned changed=1;changed<=4;changed++){
  reset();arm_request();request[18]=2;request[22]=0x6db8;checksum();hash_change=changed;
  assert(!poll()&&!release_count);
  if(changed==2){assert(committed&&arm_calls==1&&!gate7119_enabled());assert(strstr(saved_report,"\"static_snapshot_changed\":true"));}
  else assert(!committed&&!arm_calls&&mac7119_error==80);
 }
 reset();arm_request();hash_change=2;assert(!poll()&&!committed&&mac7119_error==80);
